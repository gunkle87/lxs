...
$inputPath = if ($args.Length -gt 0) { $args[0] } else { throw "usage: gen_and_fan8_macro.ps1 <input.bench> <output.bench>" }
$outputPath = if ($args.Length -gt 1) { $args[1] } else { throw "usage: gen_and_fan8_macro.ps1 <input.bench> <output.bench>" }

$lines = Get-Content $inputPath
$andNodes = New-Object System.Collections.Generic.List[object]
$sharedCounts = @{}

for ($i = 0; $i -lt $lines.Count; ++$i)
	{
	$trim = $lines[$i].Trim()
	if ($trim -match '^([^=]+)=\s*AND\(([^,]+),\s*([^)]+)\)$')
		{
		$out = $Matches[1].Trim()
		$a = $Matches[2].Trim()
		$b = $Matches[3].Trim()
		$andNodes.Add([pscustomobject]@{
			LineIndex = $i
			Output = $out
			A = $a
			B = $b
...
foreach ($shared in $groups.Keys)
	...
	for ($i = 0; $i -le ($entries.Count - 8); $i += 8)
		...
		$emitAt[$key] = "$(($chunk | ForEach-Object Output) -join ', ') = AND_FAN8($shared, $(($chunk | ForEach-Object Leaf) -join ', '))"
